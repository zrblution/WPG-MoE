...
def _disable_dropout_modules(module: nn.Module) -> Dict[str, int]:
    dropout_types = (
        nn.Dropout,
        nn.Dropout1d,
        nn.Dropout2d,
    ...
    for child in module.modules():
        if isinstance(child, dropout_types):
            if getattr(child, "p", 0.0):
                child.p = 0.0
                disabled += 1
            else:
                already_zero += 1
    return {"disabled": disabled, "already_zero": already_zero, "total": disabled + already_zero}


class WPGMoEModel(nn.Module):
    def __init__(self, config: Dict[str, object]) -> None:
        super().__init__()
        self.disable_weak_prior_inputs = bool(config.get("disable_weak_prior_inputs", False))
        self.encoder = PostEncoder(
            model_name=str(config.get("model_name", "/path/to/Qwen3.5-2B")),
            ...
            backbone_model_class=str(config.get("backbone_model_class", "auto")),
            encoder_forward_batch_size=config.get("encoder_forward_batch_size"),
        ...
        self.dropout_disable_stats = {"disabled": 0, "already_zero": 0, "total": 0}
        if bool(config.get("disable_all_dropout", False)):
            self.dropout_disable_stats = _disable_dropout_modules(self)

    def _encode_user_posts(
        self,
        risk_post_texts: List[str],
        risk_post_markers: List[str],
        block_post_texts: List[str],
        block_post_markers: List[str],
        ...
        risk_start = 0
        all_texts.extend(risk_post_texts)
        all_markers.extend(risk_post_markers)
        risk_end = len(all_texts)

        block_slice: slice | None = None
        if block_post_texts:
            block_start = len(all_texts)
            all_texts.extend(block_post_texts)
            all_markers.extend(block_post_markers)
            block_slice = slice(block_start, len(all_texts))

        segment_slices: List[slice | None] = []
        for texts, markers in zip(global_segment_texts, global_segment_markers):
            if texts:
                seg_start = len(all_texts)
                all_texts.extend(texts)
                all_markers.extend(markers)
                segment_slices.append(slice(seg_start, len(all_texts)))
            else:
                segment_slices.append(None)

        encoded_posts = self.encoder.encode_posts(all_texts, all_markers)
        risk_post_reps = encoded_posts[risk_start:risk_end]
        block_post_reps = encoded_posts[block_slice] if block_slice is not None else None
        ...
        for segment_slice in segment_slices:
            if segment_slice is None:
                ...
                segment_reps.append(encoded_posts[segment_slice].mean(dim=0))
        return risk_post_reps, block_post_reps, segment_reps
    ...
    def forward(
        ...
        *,
        ...
        risk_post_reps, block_post_reps, segment_reps = self._encode_user_posts(
            risk_post_texts,
            risk_post_markers,
            block_post_texts,
            block_post_markers,
        ...
        if self.disable_weak_prior_inputs:
        ...
        logits, h_u = self.moe_head(gate_weights, expert_outputs)
