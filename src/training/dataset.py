...
def _format_template_meta(post: dict, *, include_dims: bool) -> str:
    fields = [f"template_risk={float(post.get('risk_score', 0.0)):.2f}"]
    if include_dims:
        dims = post.get("matched_dimensions", [])
        if dims:
            fields.append("matched=" + "|".join(str(dim) for dim in dims))
    return " ".join(fields)
...
def format_user_sample(
    ...
    *,
    ...
    template_metadata_in_text: bool = False,
    template_metadata_include_dims: bool = False,
) -> dict:
    ...
    for idx, post in enumerate(risk_posts):
        ...
        elif template_metadata_in_text and "risk_score" in post:
            meta_str = _format_template_meta(post, include_dims=template_metadata_include_dims)
            risk_texts.append(f"{marker} {post['text']} {META_TOKEN} {meta_str}")
        else:
...
class UserDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self.template_metadata_in_text = template_metadata_in_text
        self.template_metadata_include_dims = template_metadata_include_dims
    ...
    def __getitem__(self, index: int) -> dict:
        return format_user_sample(
            ...
            template_metadata_in_text=self.template_metadata_in_text,
            template_metadata_include_dims=self.template_metadata_include_dims,
        )
